...
//				array[this.index + 1].fork();
			if ((this.endIndexExclusive - this.startIndexInclusive) <= SEQUENTIAL_THRESHOLD)
				...
				int currentInex = this.startIndexInclusive / SEQUENTIAL_THRESHOLD;
				ReciprocalArraySumTask left = new ReciprocalArraySumTask(
						getChunkStartInclusive(currentInex, N_TASKS, input.length),
						getChunkEndExclusive(currentInex, N_TASKS, input.length), input);
				ReciprocalArraySumTask right = new ReciprocalArraySumTask(
						getChunkEndExclusive(currentInex, N_TASKS, input.length), this.endIndexExclusive, input);
				left.fork();
				right.compute();
				left.join();
...
//				array[this.index + 1].join();
	...
	protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
		...
		SEQUENTIAL_THRESHOLD = getChunkSize(numTasks, input.length);

		ReciprocalArraySumTask main = new ReciprocalArraySumTask(0, input.length, input);
		new ForkJoinPool().invoke(main);
